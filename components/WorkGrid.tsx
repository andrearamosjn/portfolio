    ...
    <div className="px-20 max-sm:px-5">
      <nav>
        <ul className="flex flex-row flex-wrap gap-x-5">
          <li>
            <a
              className="text-2xl font-light text-[Heebo] data-[focused=true]:underline underline-offset-4 decoration-1 data-[focused=true]:font-semibold"
              href="/work"
              data-focused={id === ""}
            >
              All
          ...
          projects.map((p) => (
            <a href={`/work/${p.parent.id}/${p.id}`} key={p.id}>
              <div className="flex flex-col mb-10 cursor-pointer">
                {p.hero && <img src={p.hero} className="max-w-full" />}
                <div className="h-44 flex flex-col">
                  <span className="mt-4 text-base text-[#2e2e2e] opacity-70 uppercase font-medium leading-tight">
                    {p.parent.name}
                  </span>
                  <span className="mt-2 font-[Heebo] font-normal text-2xl uppercase leading-tight">
                    {p.name}
                  </span>
                  <span className="mt-2 text-2xl text-[#2e2e2e] font-normal leading-tight">
                    {p.description}
                  </span>
                </div>
              </div>
